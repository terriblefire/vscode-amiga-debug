...
LONG  __stdargs SetFont( struct RastPort *rp, CONST struct TextFont *textFont );
struct TextFont * __stdargs OpenFont( struct TextAttr *textAttr );
...
VOID  __stdargs CMove( struct UCopList *copList, APTR destination, LONG data );
VOID  __stdargs CWait( struct UCopList *copList, LONG v, LONG h );
...
VOID  __stdargs GfxAssociate( CONST APTR associateNode, APTR gfxNodePtr );
...
APTR  __stdargs GfxLookUp( CONST APTR associateNode );
...
ULONG  __stdargs GetDisplayInfoData( CONST DisplayInfoHandle handle, APTR buf, ULONG size, ULONG tagID, ULONG displayID );
...
VOID  __stdargs GetRPAttrsA( CONST struct RastPort *rp, CONST struct TagItem *tags );
VOID  __stdargs GetRPAttrs( CONST struct RastPort *rp, ULONG tag1Type, ... );
ULONG  __stdargs BestModeIDA( CONST struct TagItem *tags );
ULONG  __stdargs BestModeID( ULONG tag1Type, ... );
/*--- functions in V40 or higher (Release 3.1) ---*/
VOID  __stdargs WriteChunkyPixels( struct RastPort *rp, ULONG xstart, ULONG ystart, ULONG xstop, ULONG ystop, CONST UBYTE *array, LONG bytesperrow );
