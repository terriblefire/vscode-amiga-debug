...
struct Task * __stdargs CreateTask( CONST_STRPTR name, LONG pri, CONST APTR initPC, ULONG stackSize );
...
VOID  __stdargs arnd( LONG place, LONG exp, STRPTR string );
FLOAT  __stdargs dbf( ULONG exp, ULONG mant );
LONG  __stdargs fpa( FLOAT fnum, BYTE *string );
VOID  __stdargs fpbcd( FLOAT fnum, BYTE *string );

/* Timer support functions in amiga.lib (V36 and higher only) */

LONG  __stdargs TimeDelay( LONG unit, ULONG secs, ULONG microsecs );
LONG  __stdargs DoTimer( struct timeval *, LONG unit, LONG command );
