...
LONG  __stdargs WriteChunkBytes( struct IFFHandle *iff, CONST APTR buf, LONG numBytes );
LONG  __stdargs ReadChunkRecords( struct IFFHandle *iff, APTR buf, LONG bytesPerRecord, LONG numRecords );
LONG  __stdargs WriteChunkRecords( struct IFFHandle *iff, CONST APTR buf, LONG bytesPerRecord, LONG numRecords );
...
struct StoredProperty * __stdargs FindProp( CONST struct IFFHandle *iff, LONG type, LONG id );
struct CollectionItem * __stdargs FindCollection( CONST struct IFFHandle *iff, LONG type, LONG id );
struct ContextNode * __stdargs FindPropContext( CONST struct IFFHandle *iff );
struct ContextNode * __stdargs CurrentChunk( CONST struct IFFHandle *iff );
struct ContextNode * __stdargs ParentChunk( CONST struct ContextNode *contextNode );
...
APTR  __stdargs LocalItemData( CONST struct LocalContextItem *localItem );
VOID  __stdargs SetLocalItemPurge( struct LocalContextItem *localItem, CONST struct Hook *purgeHook );
VOID  __stdargs FreeLocalItem( struct LocalContextItem *localItem );
struct LocalContextItem * __stdargs FindLocalItem( CONST struct IFFHandle *iff, LONG type, LONG id, LONG ident );
...
VOID  __stdargs InitIFF( struct IFFHandle *iff, LONG flags, CONST struct Hook *streamHook );
