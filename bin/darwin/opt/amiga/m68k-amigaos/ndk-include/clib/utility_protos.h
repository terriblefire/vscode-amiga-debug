...
ULONG  __stdargs UnpackStructureTags( CONST APTR pack, CONST ULONG *packTable, struct TagItem *tagList );
...
struct NamedObject * __stdargs AllocNamedObject( CONST_STRPTR name, Tag tag1, ... );
LONG  __stdargs AttemptRemNamedObject( struct NamedObject *object );
struct NamedObject * __stdargs FindNamedObject( struct NamedObject *nameSpace, CONST_STRPTR name, struct NamedObject *lastObject );
